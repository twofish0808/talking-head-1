...
def find_dir(path):
    for fd in os.listdir(path):
        ...
        if fd.split('_')[-1] == '0.png':
            for _ in range(67,141):
                imgList.append(full_path)
        elif fd.split('.')[-1] == 'png' and int(fd.split('_')[-1].split('.')[0])<=141 and int(fd.split('_')[-1].split('.')[0])>67:
            print(full_path)
...
def find_dir(path):
    for fd in os.listdir(path):
        full_path = os.path.join(path, fd)
        if fd.split('.')[-1] == 'txt' and int(fd.split('_')[-1].split('.')[0])<=141 and int(fd.split('_')[-1].split('.')[0])>67:
            a = []
